...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            gen_regression({}),
            EigenProRegressor(
                kernel="rbf", n_epoch=100, random_state=1
            ),
        ),
        ...
        (
            gen_regression({}),
            EigenProRegressor(
                kernel="laplace", n_epoch=100, gamma=.008, random_state=1
            ),
        ),
        # Test cauchy kernel
        (
        ...
        (
            ...
            EigenProRegressor(
                kernel="rbf", n_epoch=100, gamma=.003, random_state=1
            ),
        ),
        ...
        (
            ...
            EigenProRegressor(
                kernel="rbf", n_epoch=100, gamma=.5, random_state=1
            ),
        ),
        ...
        (
            ...
            EigenProRegressor(
                kernel="rbf", n_epoch=60, gamma=.005, random_state=1
            ),
        ),
    ],
)
...
def test_eigenpro_regression_duplicate_data():
    ...
    prediction = (
        EigenProRegressor(
            kernel="rbf", n_epoch=100, gamma=.02, random_state=1
        )
        .fit(X, y)
        .predict(X)
    )
...
def test_eigenpro_regression_conflict_data():
    ...
    EigenProRegressor(
        kernel="linear", n_epoch=5, gamma=.5, random_state=1
    ).fit(X, y).predict(X)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            gen_classification({}),
            EigenProClassifier(
                kernel="laplace", n_epoch=100, gamma=.003, random_state=1
            ),
        ),
        # Test cauchy kernel
        (
            gen_classification({}),
            EigenProClassifier(
                kernel="cauchy", n_epoch=100, gamma=.005, random_state=1
            ),
        ),
        # Test with a very large number of input features
        ...
        (
            ...
            EigenProClassifier(
                kernel="rbf", n_epoch=100, gamma=.0012, random_state=1
            ),
        ),
        ...
        (
            ...
            EigenProClassifier(
                kernel="rbf", n_epoch=200, gamma=.008, random_state=1
            ),
        ),
        ...
        (
            ...
            EigenProClassifier(
                kernel="laplace", n_epoch=100, gamma=.0012, random_state=1
            ),
        ),
    ],
)
...
def test_eigenpro_classification_duplicate_data():
    ...
    prediction = (
        EigenProClassifier(
            kernel="rbf", n_epoch=60, gamma=.02, random_state=1
        )
        .fit(X, y)
        .predict(X)
    )
...
def test_eigenpro_classification_conflict_data():
    ...
    # Make sure we don't throw an error when fitting or predicting
    EigenProClassifier(
        kernel="linear", n_epoch=5, gamma=.02, random_state=1
    ).fit(X, y).predict(X)

...
@pytest.mark.parametrize('params, err_msg', [
    ...
    ({'subsample_size': -1}, 'subsample_size should be non-negative, was -1'),
    ...
    [
        ...
        (gen_regression({'n_features': 10000}),
            FKR_EigenPro(kernel="gaussian", n_epoch=100,
                         bandwidth=1, random_state=1)),
        # Test a very simple underlying distribution
        (gen_regression({'n_informative': 1}),
            FKR_EigenPro(batch_size=500, kernel="gaussian", n_epoch=100,
                         bandwidth=10, random_state=1)),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (gen_classification({'n_samples': 10, 'hypercube': False}),
            FKC_EigenPro(batch_size=9, kernel="gaussian", bandwidth=2.5,
                         n_epoch=100, random_state=1)),
        # Test laplacian kernel
        (gen_classification({}), FKC_EigenPro(
            kernel="laplace", n_epoch=100, bandwidth=13,
            random_state=1)),
        # Test cauchy kernel
        (gen_classification({}), FKC_EigenPro(
        ...
        (gen_classification({'n_samples': 500, 'n_features': 500,
                            'n_informative': 160, 'scale': 30, 'shift': 6}),
         FKC_EigenPro(kernel="gaussian", n_epoch=100,
                      bandwidth=20, random_state=1)),
        # Test a distribution that has been shifted
        (gen_classification({'shift': 1, 'hypercube': False}),
         FKC_EigenPro(kernel="gaussian", n_epoch=200, bandwidth=8,
                      random_state=1)),
        # Test with many redundant features.
        (gen_classification({'n_redundant': 18}),
         FKC_EigenPro(kernel="laplace", n_epoch=100, bandwidth=20,
                      random_state=1))
    ]
)

...
optics = cluster.OPTICS(min_samples=20,
                            xi=0.1,
                            min_cluster_size=.2)
affinity_propagation = cluster.AffinityPropagation(damping=.75, preference=-220)
...
for N in [300,3000]: 
    ...
    bandwidth = cluster.estimate_bandwidth(X, .2)
    ...
    plt.subplots_adjust(left=.02, right=.98, bottom=.001, top=.85, wspace=.05,
                    hspace=.18)
    ...
    for name, algorithm in clustering_algorithms:
        ...
        colors = np.array(list(islice(cycle(['#377eb8', '#ff7f00', '#4daf4a',
                                      ...
                                      int(max(y_pred) + 1))))
        # add black color for outliers (if any) as computed by OPTICS and DBSCAN
        ...
        plt.suptitle('Dataset with '+str(N)+' samples, '
                     +str(int(N/100))+' outliers.', size=20)
        plot_num += 1
